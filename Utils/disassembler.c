    ...
    switch (typeOP) {
        ...
        switch (typeOP) {
            ...
            case MEMORY_OPERAND: {
                ...
                else if (offset < 0) printf("%d", offset);
                printf("]");
...
void dissasembler(cpu_t cpu, mem_t mem){
    ...
    if (mem.segment_count > 1){
        ...
        if (cpu.KS != 0xFFFFFFFF){
            ...
            while (offset < ks_size) {
                ...
                int len = 0;
                while ((offset + len) < ks_seg->size && mem.data[str_phys_addr + len] != '\0') {
                    len++;
                }
                len++; // P incluir el \0
        ...
        if (cpu.CS != 0xFFFFFFFF){
            ...
            uint32_t entry_point_offset = offset = cpu.IP; // IP lógico V2, esta bien q el offset arranque aca no?

            while (offset < cs_seg->size) {
                printf("%c", (offset == entry_point_offset) ? '>' : ' '); // Entry point
                printf("[%04X] ", offset);

                uint32_t physical_addr = cs_seg->base + offset;
